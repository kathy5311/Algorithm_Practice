...
# ---- BFS vs DFS 비교 ----
# 탐색 순서:
#   BFS → 층 단위 (1 → 2,3 → 4,5,6)   자료구조: 큐(deque)
#   DFS → 깊이 우선 (1→2→4→5→3→6)     자료구조: 스택 / 재귀
#
# 경로 특성:
#   BFS → 처음 도달한 경로 = 항상 최단 경로 보장
#   DFS → 최단 경로 보장 안 함
#
# 언제 쓰나:
#   "몇 번 만에 도달하나?" (최단 거리/이동 횟수)    → BFS
#   "가능한 모든 경우를 탐색하라" (경우의 수/조합)  → DFS
#
# 문제 유형별 선택:
#   최단 거리 / 최소 이동 횟수                → BFS  (처음 도달 = 최단)
#   가중치 없는 격자 최단 경로 (상하좌우)      → BFS
#   연결 요소 개수, 방문 여부만 체크           → BFS/DFS 둘 다 가능
#   경우의 수 / 모든 조합·순열 탐색           → DFS  (백트래킹과 결합)
#   사이클 감지                               → DFS  (재귀 콜스택으로 추적)
#   트리 전위·중위·후위 순회                   → DFS
#
# 대표 문제:
#   게임 맵 최단거리  → BFS   타겟 넘버      → DFS
#   단어 변환        → BFS   네트워크       → DFS/BFS
#   미로 탈출        → BFS   올바른 괄호 생성 → DFS (백트래킹)

# ---- 투 포인터 vs 슬라이딩 윈도우 ----
# 핵심 차이: 윈도우 크기가 고정이냐 아니냐
#
# [투 포인터] 윈도우 크기 가변, 조건에 따라 left/right 독립 이동
#   - 정렬 후 두 값 조합 탐색 (두 수의 합, 3Sum)
#   - 조건 만족할 때까지 늘리고 줄이는 부분 배열
#   - 팰린드롬 확인, 연결 리스트 사이클 (fast/slow)
#   - 포인터 방향: 양끝에서 좁혀오기 또는 같은 방향(fast/slow)
#
# [슬라이딩 윈도우] 윈도우 크기 고정 k, 순서 유지 (정렬 불필요)
#   - 길이 k 부분 배열의 최대/최소/합
#   - 아나그램 탐색, 고정 크기 문자열 패턴
#   - 포인터 방향: left/right 항상 함께 이동
#
# 판단 기준:
#   크기 k 고정, 연속 구간 최대/최소  → 슬라이딩 윈도우
#   조건 만족까지 늘리고 줄이기        → 투 포인터
#   정렬 후 두 값 조합 찾기            → 투 포인터
#   문자열에서 고정 크기 패턴 탐색      → 슬라이딩 윈도우
#
# 주의: 합이 k 이상인 최단 구간처럼 슬라이딩처럼 보여도
#       윈도우 크기가 조건에 따라 변하면 → 투 포인터
